...
function [G, S] = slotted_aloha(HostNum, PacketNum)
    ...
    for hosts = 2:HostNum
        ...
        % 產生隨機時間點 配和 slotted來產生 %
        randTimePoint = cumsum(slotted(factor* rand(PacketNum, hosts), frameTime));
        ...
        for i = 1:hosts*PacketNum - 1
            ...
            %{ 
                和原本的ALOHA不同的地方，
                因為最初產生時間時已經進行slotted，
                只要時間間隔大於timeFrame，就不會發生衝撞。
            %}

            if interval(1, i) >= frameTime
